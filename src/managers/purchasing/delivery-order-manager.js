...
module.exports = class DeliveryOrderManager extends BaseManager {
    constructor(db, user) {
        super(db, user);
        this.collection = this.db.use(map.purchasing.DeliveryOrder);
    ...
    _beforeInsert(deliveryOrder) {
        deliveryOrder.refNo = generateCode(moduleId);
    ...
    _createIndexes() {
        var dateIndex = {
            name: `ix_${map.purchasing.DeliveryOrder}_date`,
            key: {
                date: -1
            }
        }
        ...
        var refNoIndex = {
            name: `ix_${map.purchasing.DeliveryOrder}_refNo`,
            key: {
                refNo: 1
            },
            unique: true
