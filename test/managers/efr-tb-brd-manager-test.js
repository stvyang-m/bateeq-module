...
before('#00. connect db', function (done) {
    helper.getDb()
        .then(db => {
            ...
            data(db)
                .then(result => {
                    var PusatReturTokoTerimaBarangReturSelesaiPerbaikanManager = require('../../src/managers/inventory/efr-tb-brd-manager');
                    manager = new PusatReturTokoTerimaBarangReturSelesaiPerbaikanManager(db, {
                        username: 'unit-test'
                    });
                    testData = result;
                    done();
